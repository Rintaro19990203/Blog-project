...
@app.route("/logged-home/<user_id>")
@login_required
def logged_home(user_id):
    posts = BlogPost.query.all()
    return render_template("index.html", all_posts=posts, logged_in=True, id=int(user_id))
...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        try:
            ...
            login_user(user)
            return redirect(url_for("logged_home", user_id=user.id))
...
@app.route("/post/<int:post_id>/<user_id>", methods=["GET", "POST"])
def show_post(post_id, user_id):
    ...
    if user_id == 0:
        return render_template("post.html", post=requested_post, comment_form=comment_form,
                               id=user_id, logged_in=False, comments=comments, post_id=post_id)
    else:
        if comment_form.validate_on_submit():
            data = comment_form.comment.data
            comment = Comment(
                text=data,
            ...
            db.session.commit()
            return redirect(url_for("show_post", post_id=post_id, user_id=user_id))
        return render_template("post.html", post=requested_post, comment_form=comment_form,
                               id=int(user_id), logged_in=True, comments=comments, post_id=post_id)
